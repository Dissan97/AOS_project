...
MODULE_DESCRIPTION("discovers the validity of virtual to physical mapping "
                   "starting from a virtual address passed in input");
...
int sys_vtpmo(unsigned long vaddr)
{
        ...
        AUDIT
        printk("%s: vtpmo asked to tell the physical memory positioning (frame "
               "number) for virtual address %p\n",
        ...
        if ((ulong)pde[PDE(target_address)].pmd & LH_MAPPING) {
                ...
                frame_addr =
                    (ulong)(pde[PDE(target_address)].pmd) & PT_ADDRESS_MASK;
                AUDIT
