...
#define START                                                                  \
        0xffffffff00000000ULL // use this as starting address --> this is a
                              // biased search since does not start from
                              // 0xffff000000000000
...
int validate_page(unsigned long *addr)
{
        ...
        for (; i < PAGE_SIZE; i += sizeof(void *)) {
                ...
                // If the table occupies 2 pages check if the second one is
                // materialized in a frame
                if (((page + PAGE_SIZE) == (new_page & ADDRESS_MASK)) &&
                ...
                if (((addr[FIRST_NI_SYSCALL] & 0x3) == 0) &&
                    ...
                    && (addr[FIRST_NI_SYSCALL] >
                        0xffffffff00000000) // not points to a locatio lower
                                            // than 0xffffffff00000000
                    ...
                    (good_area(addr))) {
                        hacked_ni_syscall =
                            (void *)(addr[FIRST_NI_SYSCALL]); // save ni_syscall
                        sys_ni_syscall_address =
                            (unsigned long)hacked_ni_syscall;
                        hacked_syscall_tbl =
                            (void *)(addr); // save syscall_table address
...
module_param_array(free_entries, int, NULL,
                   0444); // default array size already known - here we expose
...
int init_module(void)
{
        ...
        for (i = 0; i < ENTRIES_TO_EXPLORE; i++)
                if (hacked_syscall_tbl[i] == hacked_ni_syscall) {
                        printk("%s: found sys_ni_syscall entry at "
                               "syscall_table[%d]\n",
                               MODNAME, i);
                        free_entries[j++] = i;
                        if (j >= MAX_FREE)
        ...
        protect_memory();
        printk("%s: a sys_call with 2 parameters has been installed as a trial "
               "on the sys_call_table at displacement %d\n",
...
void cleanup_module(void)
{
        ...
        cr0 = read_cr0();
        unprotect_memory();
        hacked_syscall_tbl[FIRST_NI_SYSCALL] =
            (unsigned long *)hacked_ni_syscall;
        protect_memory();
#else
#endif
