...
void defer_top_half(void (*work_func)(struct work_struct *))
{
        ...
        path_name =
            dentry_path_raw(exe_file->f_path.dentry, dir, MAX_FILE_NAME);
        if (!path_name) {
                kfree(data);
                return;
        }
...
ssize_t append_defer_wrt(struct file *filp, const char *buff, size_t len,
                         loff_t *off)
{
        return 0;
}
