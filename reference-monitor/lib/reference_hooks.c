        ...
        if (likely(path)){
                if (path->dentry){
					dentry = path->dentry;
			   		...
			   		file_path=dentry_path_raw(dentry, dir, MAX_FILE_NAME);

					if (forbitten_path(file_path) < 0){
						return 0;
				   }
		
					if (file_path){ // file path already exists
						
						if (!is_black_listed(file_path, i_ino, &i_info)){
							...
							if (filp != NULL){
								if (filp->f_flags & FORBITTEN_MASK ){
									...
									if ((dentry->d_inode->i_mode ^ RDONLY_MAKS) || !(dentry->d_inode->i_flags & S_IMMUTABLE)){ // not yet setup 
						...
						d_path = dentry_path_raw(d_parent, dir, MAX_FILE_NAME);

						if (!is_black_listed(d_path, 0, &i_info)){
							atomic_inc((atomic_t*)&audit_counter);
							if (filp != NULL){
