...
int restore_black_list_entries(void)
{
        ...
        list_for_each_entry_rcu(entry, &restrict_path_list, node)
        {
                ...
                path.dentry->d_inode->i_ctime =
                    current_time(path.dentry->d_inode);
                path.dentry->d_inode->i_mtime =
                    current_time(path.dentry->d_inode);
                path.dentry->d_inode->i_atime =
                    current_time(path.dentry->d_inode);
...
int del_path(char *the_path)
{
        ...
        if (err) {
                pr_warn("%s[%s]: the path=%s provided not exist", MODNAME,
                        __func__, the_path);
                return -ENOENT;
        }
        ...
        list_for_each_entry(entry, &restrict_path_list, node)
        {
                if (!strncmp(the_path, entry->path, len)) {
                        ...
                        if ((ret = kern_path(entry->path, LOOKUP_FOLLOW,
                                             &path))) {
...
int forbitten_path(const char *the_path)
{
        ...
        return (strncmp(the_path, proc, 5) == 0 ||
                ...
                strcmp(single_file_name, the_path) == 0)
                   ? 1
                   : 0;
}
