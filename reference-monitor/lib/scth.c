 ...
 * 	 the dynamis discovery of the position of the sys_call_table and the
 * identifiction of entries pointing to sys_ni_syscall - these entries are
 * returned together with the address of the sys_call_table and the address of
 * sys_ni_syscall so that they can be exploited for rewriting sys_call_table and
 * adding new system calls to Linux
 *
                ...
                unsigned long *sys_ni_sys_call)
{
        ...
        printk("%s: trying to get %d entries from the sys-call table at "
               "address %px\n",
        ...
        for (i = 0; i < 256; i++) {
                for (z = i + 1; z < 256; z++) {
                        if (p[i] == p[z]) {
                                AUDIT
                                {
                                        printk("%s: table entries %d and %d "
                                               "keep the same address\n",
                                               LIBNAME, i, z);
                                        printk("%s: sys_ni_syscall correctly "
                                               "located at %px\n",
                                               LIBNAME, (void *)p[i]);
                                }
                                addr = p[i];
                                ...
                                for (k = z + 1;
                                     k < 256 && j < (num_acquires - 1); k++) {
                                        if (p[i] == p[k]) {
                                                printk("%s: acquiring table "
                                                       "entry %d\n",
                                                       LIBNAME, k);
                                                restore[++j] = k;
                                                ret++;
