...
void test_change_state(char *good_password, char *bad_password, struct sys_function_pointer syscall_function_pointer){
    int i = 0;
    int read_state;
    seteuid(1000);
    // good argment but bad user
    int ret;
    assert((ret = syscall_function_pointer.change_state_func(good_password, 0x1)) < 0);
    ...
    for (i =0 ; bad_state[i] != -1000; i++){
        ...
        fflush(stdout);
        assert(syscall_function_pointer.change_state_func(bad_password, bad_state[i]) < 0);
        perror("bad password bad bad_state expected EINVAL");
        assert(errno == EINVAL);    
        perror("good password bad_state expected EINVAL");
        assert(syscall_function_pointer.change_state_func(good_password, bad_state[i]) < 0);
        assert(errno == EINVAL);
    }
    ...
    assert(syscall_function_pointer.change_state_func(good_password, all_state[i]) < 0);
...
void test_change_path(char *good_password, char *bad_password, node_list_t file_list, struct sys_function_pointer syscall_function_pointer) {
    int ret;
    int expected;
    ...
    fflush(stdout);
    ret = syscall_function_pointer.change_path_func(good_password, NULL, ADD_PATH);
    perror("change path invalid path=NULL expected EINVAL");

    assert (ret < 0 && errno == EINVAL);
    ret = syscall_function_pointer.change_path_func(good_password, file_list.head->path, (ADD_PATH + 1000));
    perror("change path invalid op=ADD_PATH+1000 expected EINVAL");
    assert (ret < 0 && errno == EINVAL);

    ret = syscall_function_pointer.change_path_func(bad_password, file_list.head->path, (ADD_PATH  & REMOVE_PATH));
    ...
    for (int i = 0; forbitten_paths[i] != NULL; i++){
        ret = syscall_function_pointer.change_path_func(good_password, forbitten_paths[i], ADD_PATH );
    ...
    ret = syscall_function_pointer.change_path_func(bad_password, file_list.head->path, ADD_PATH);
    ...
    ret = syscall_function_pointer.change_path_func(good_password, file_list.head->path, ADD_PATH);
    perror("change path invalid op=ADD_PATH and state != REC_ON && state != REC_ON REC_OFF expected ECANCELED");
    assert (ret < 0 && errno == ECANCELED);

    ret = syscall_function_pointer.change_path_func(good_password, file_list.head->path, REMOVE_PATH);
    perror("change path invalid op=REMOVE_PATH and state != REC_ON && state != REC_ON REC_OFF expected ECANCELED");
    assert (ret < 0 && errno == ECANCELED);
...
int main(int argc, char **argv){
    ...
    test_change_state(password.good_password, password.bad_password, syscall_function_pointer);
    ...
    test_change_path(password.good_password, password.bad_password, cngpth_mock.list, syscall_function_pointer);
