...
node_t *create_node(const char *path, int absolute, int id_dir, int add,
...
{
    node_t *new_node = create_node(path, absolute, id_dir, add, is_hardlink,
                                   which_file, create);
    if (!new_node) {
...
void print_list(const node_list_t *list)
{
    ...
    current = list->head;
    while (current) {
        printf("Path: %s, Absolute: %d, ID: %d, Add: %d, Is Hardlink: "
               "%d, Which File: %s, create: %d\n",
               current->path, current->absolute, current->id_dir, current->add,
               current->is_hardlink,
               current->which_file ? current->which_file : "NULL",
               current->create);
